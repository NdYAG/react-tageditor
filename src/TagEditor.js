...
class TagEditor extends Component {
  ...
  handleClick() {
    this.props.store.add(
      "",
      function(err, newTag, indexOfNewTag) {
        this.setState({
  ...
  handleTagSave(tag, text) {
    this.props.store.save(
      ...
      function(err) {
        if (!err) {
          this.props.onChange &&
            this.props.onChange(text, this.props.store.output(), "add");
          return;
        }
        switch (err.name) {
          case ERROR.EMPTY:
  ...
  handleTagRemove(tag) {
    ...
    tagText.length &&
      this.props.onChange &&
      this.props.onChange(tagText, this.props.store.output(), "remove");
  }
  handleTagSplit(tag, textBeforeCaret, textAfterCaret) {
    this.props.store.insertAfterTag(
      tag,
      textAfterCaret,
      function(err, newTag, indexOfNewTag) {
        this.setState({
  ...
  render() {
    ...
    var tagNodes = tags.map((tag, i) => {
      return (
        <Tag
          ...
          currentTagTxt={tagInput => {
            this.props.currentTagTxt(tagInput);
          }}
          delimiterKeys={this.props.delimiters.filter(d => {
            return typeof d === "number";
          })}
          delimiterChars={this.props.delimiters.filter(d => {
    ...
    if (!tags.length) {
      tagNodes = (
        <div className="tag-editor-placeholder">{this.props.placeholder}</div>
      );
    }
    return (
      <div
        className={
          "tag-editor" +
          (typeof this.state.editing === "number" ? " is-active" : "")
        }
